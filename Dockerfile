# UltraBoost Dashboard Dockerfile
# Powered by BoostNova
# Set the base image
FROM continuumio/miniconda3:latest AS builder
# Install system dependencies
RUN apt-get update && \
    apt-get install -y sudo libusb-1.0 python3-dev gcc && \
    rm -rf /var/lib/apt/lists/*
WORKDIR /home/ultraboost-dashboard
# Create conda environment
COPY environment_conda.yml /tmp/environment_conda.yml
RUN conda env create -f /tmp/environment_conda.yml && \
    conda clean -afy && \
    rm /tmp/environment_conda.yml
# Copy remaining files
COPY . .
SHELL [ "/bin/bash", "-lc" ]
RUN echo "conda activate ultraboost-dashboard" >> ~/.bashrc
# Build final image using artifacts from builder
FROM continuumio/miniconda3:latest AS release
# Dockerfile author / maintainer
LABEL maintainer="BoostNova"
LABEL description="UltraBoost Dashboard - Algorithmic Trading Control Center"
# Build arguments
ARG BRANCH=""
ARG COMMIT=""
ARG BUILD_DATE=""
LABEL branch=${BRANCH}
LABEL commit=${COMMIT}
LABEL date=${BUILD_DATE}
# Set ENV variables
ENV COMMIT_SHA=${COMMIT}
ENV COMMIT_BRANCH=${BRANCH}
ENV BUILD_DATE=${BUILD_DATE}
# Install system dependencies
RUN apt-get update && \
    apt-get install -y curl && \
    rm -rf /var/lib/apt/lists/*
# Create mount points
RUN mkdir -p /home/ultraboost-dashboard/data
WORKDIR /home/ultraboost-dashboard
# Copy all build artifacts from builder image
COPY --from=builder /opt/conda/ /opt/conda/
COPY --from=builder /home/ /home/
EXPOSE 8501
# Setting bash as default shell because we have .bashrc with customized PATH
SHELL [ "/bin/bash", "-lc" ]
# Set the default command to run when starting the container
CMD conda activate ultraboost-dashboard && streamlit run main.py